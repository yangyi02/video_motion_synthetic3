...
class Net(nn.Module):
    ...
    def forward(self, im_input, im_output):
        ...
        seg = out_mask.sum(1)
        appear = F.relu(1 - seg)
        occl = seg2occl(depth, out_mask, self.m_kernel, self.m_range)
        im = im_input[:, -self.im_channel:, :, :]
        pred, unocclude = construct_image(im, out_mask, occl, self.m_kernel, self.m_range)
        ...
        pred_depth, _ = construct_image(depth, out_mask, occl, self.m_kernel, self.m_range)
...
def seg2occl(depth, out_mask, m_kernel, m_range):
    depth_expand = depth.expand_as(out_mask)
    nearby_depth = F.conv2d(depth_expand, m_kernel, None, 1, m_range, 1, m_kernel.size(0))
    depth_max, _ = torch.max(nearby_depth * out_mask, 1)
    occl = F.relu(nearby_depth * out_mask - depth_max.expand_as(nearby_depth) + 1)
    return occl


def construct_image(im, out_mask, occl, m_kernel, m_range):
    unocclude_mask = out_mask * occl
